...
//        }
    ...
    public static boolean initUdw(final Context currentContext, final int position, final GenericUdw<?> el, final boolean maximized) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                Method method = null;
...
//					if (D) Log.d(TAG, "initUdw is rsm null? " + (rsm == null));
                    ...
                    method.invoke(view, null, el.id, el.initParams, position,
    ...
    public void callInit(Context context, int position, boolean maximized) {
        initUdw(context, position, this, maximized);
